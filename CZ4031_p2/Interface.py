...
class BuildWindow(tk.Tk):
  ...
  def __init__(self, *args, **kwargs):
    ...
    connect_btn = tk.Button(self, text = "Connect", font = ("Century Gothic", 9), command = lambda :self.submit(self.hostNameEntry.get(), self.portEntry.get(),self.dbNameEntry.get(),self.userNameEntry.get(),self.passwordEntry.get()))
    ...
    self.generateQueryBtn = tk.Button(self, text = "Generate Query Plan", font = ("Century Gothic", 9), command = lambda :self.generate(self.queryScrollText.get("1.0", tk.END)))
